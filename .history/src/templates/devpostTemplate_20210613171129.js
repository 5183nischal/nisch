  ...
  if (items) {
    return (
      ...
      >
        {items.map((item) => {
          return (
            <li key={item.title} className="my-px8 ml-px16 pl-px10 text-orange">
              <a
                href={item.url}
                className={`border-b ${
                  depth % 2 == 0 ? "border-orange" : ""
                } hover:border-white`}
              >
                {item.title}
              </a>
...
const components = {
  p: (props) => (
    <p
      {...props}
      className="my-px30 text-1r leading-1p7em hyphens-auto md:text-1p04r md:leading-1p75em"
    />
  ),
  h2: (props) => (
    <h2
      {...props}
      className="mt-px50 -mb-px10 text-1p5r text-orange tracking-npx1 font-helvetica hyphens-auto text-black"
    ></h2>
  ),
  h3: (props) => (
    <h3
      {...props}
      className="mt-px40 -mb-px15 text-1p1r text-orange tracking-npx1 font-helvetica font-semibold hyphens-auto text-black"
    ></h3>
  ),
  ...
  blockquote: (props) => (
    <blockquote {...props} className="font-helvetica text-p95r text-orange font-italic"></blockquote>
  ),
  a: (props) => (
    <a {...props} className="border-b border-orange hover:border-white text-orange"></a>
  ),
  inlineCode: (props) => (
    <code
      {...props}
...
export default function DevPostTemplate({ data: { mdx } }) {
  const { title, images, videos } = mdx.frontmatter;
  ...
  return (
    <LayoutWhite writingInfo={infoCard}>
      <SEO title="Essays" description="nisch essays" />
      <section className="mx-2/25 flex-1 md:m-px50">
        <div className="max-w-px1000">
          <h1 className="mt-px20 mb-px40 pb-px40 text-4p5r text-orange font-thin leading-1em tracking-npx6 hyphens-auto text-black border-b-8 border-black md:mb-px50 md:pb-px50 md:border-b-10 md:text-5p75r md:hyphens-none lg:mb-px50 lg:pb-px50">
            {title}
          </h1>
        </div>
          ...
          {mdx.tableOfContents.items ? (
            <div className="md:float-right md:ml-px30 md:w-px200">
              <div className="text-p65r font-helvetica leading-1p5em">
                <h2 className="my-px10 text-p7r tracking-npxp5 text-orange font-bold uppercase">
                  Contents
                </h2>
